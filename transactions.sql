...
DATETIME(EXTRACT(YEAR FROM PARSE_DATE("%Y%m%d", date)), EXTRACT(MONTH FROM PARSE_DATE("%Y%m%d", date)),
         EXTRACT(DAY FROM PARSE_DATE("%Y%m%d", date)), hits.hour, hits.minute, 00) AS datetime, SUM(totals.visits) AS visits
...
product_visits AS (
SELECT fullvisitorid, v2ProductName, datetime, SUM(visits) 
OVER(PARTITION BY fullvisitorid, v2ProductName ORDER BY datetime ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS product_visits FROM product),
...
category_visits AS (SELECT fullvisitorid, v2ProductCategory, datetime,
...
AND transactions.v2ProductName = product_visits.v2ProductName AND transactions.datetime_transaction = product_visits.datetime
